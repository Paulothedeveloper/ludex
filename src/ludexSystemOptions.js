...
export const SYSTEM_OPTIONS = {
  // ===== PS2 (PCSX2 / LRPS2) =====
  // v0.9.1: catalogo MUITO expandido pra dar liberdade total. Defaults
  // conservadores pra PERFORMANCE (1x native, bilinear, sem aniso) — user
  // sobe se a maquina aguentar.
  ps2: [
    sel('pcsx2_upscale_multiplier', 'Resolução', 'video',
      ['1x Native (PS2)', '2x Native (~720p)', '3x Native (~1080p)', '4x Native (~1440p/2K)', '6x Native (~2160p/4K)', '8x Native (~3240p)'],
      '1x Native (PS2)'),
    ...
    sel('pcsx2_anisotropic_filtering', 'Filtro Anisotrópico', 'video',
      ...
      'disabled'),
    sel('pcsx2_mipmapping', 'Mipmapping', 'video',
      ['Automatic', 'Off', 'Basic', 'Full'],
      'Automatic'),
    sel('pcsx2_dithering', 'Dithering', 'video',
      ['Unscaled', 'Scaled', 'Off'],
      'Unscaled'),
    sel('pcsx2_blending_accuracy', 'Precisão Blending', 'video',
    ...
    sel('pcsx2_deinterlace_mode', 'Desentrelaçamento', 'video',
      ...
      'Automatic'),
    sel('pcsx2_pgs', 'Precisão GS (geometria)', 'video',
      ['Disabled', 'Promote sprites to triangles', 'Quad to triangle', 'Edge AA1'],
      'Disabled'),
    sel('pcsx2_renderer', 'Renderer', 'video',
    ...
    sel('pcsx2_ee_cycle_rate', 'Velocidade CPU (EE)', 'performance',
      ...
      '100% (Normal Speed)'),
    sel('pcsx2_ee_cycle_skip', 'EE Cycle Skip', 'performance',
      ['Disabled', 'Mild Underclock', 'Moderate Underclock', 'Maximum Underclock'],
      'Disabled'),
    sel('pcsx2_vu_cycle_steal', 'VU Cycle Steal', 'performance',
      ['Disabled', '1', '2', '3'],
      'Disabled'),
    ...
    sel('pcsx2_uncapped_framerate_hint', 'FPS sem Limite', 'performance',
      ...
      'disabled'),
    sel('pcsx2_mtvu', 'Multi-Threaded VU1 (perf)', 'performance',
      ['enabled', 'disabled'],
      'enabled'),
    sel('pcsx2_audio_volume', 'Volume Áudio Core', 'audio',
      ['25', '50', '75', '100', '125', '150', '200'],
      '100'),
    sel('pcsx2_audio_synchmode', 'Sync Áudio', 'audio',
      ['TimeStretch', 'Async Mix', 'None'],
      'TimeStretch'),
    sel('pcsx2_audio_interpolation', 'Interpolação Áudio', 'audio',
      ['Nearest', 'Linear', 'Cubic', 'Hermite', 'Catmull-Rom', 'Gaussian'],
      'Catmull-Rom'),
    sel('pcsx2_audio_expansion_mode', 'Expansão Áudio', 'audio',
      ['Stereo', 'Quadraphonic', 'Surround 5.1', 'Surround 7.1'],
      'Stereo'),
    sel('pcsx2_enable_widescreen_patches', 'Patches Widescreen (jogo)', 'video',
      ['disabled', 'enabled'],
      'disabled'),
    sel('pcsx2_enable_no_interlacing_patches', 'Patches Sem Entrelaçamento', 'video',
      ['disabled', 'enabled'],
      'disabled'),
    sel('pcsx2_enable_cheats', 'Habilitar Cheats', 'sistema',
      ['disabled', 'enabled'],
      'disabled'),
    sel('pcsx2_memory_card_slot_1', 'Memory Card Slot 1', 'sistema',
      ['empty', 'shared (8MB)', 'dedicated (per-game)'],
      'dedicated (per-game)'),
  ],
  ...
  // v0.9.1: expandido (era 10, agora 18)
  wii: [
    sel('dolphin_efb_scale', 'Resolução Interna', 'video',
      ['x1 (640 x 528)', 'x2 (1280 x 1056)', 'x3 (1920 x 1584)', 'x4 (2560 x 2112)', 'x5 (3200 x 2640)', 'x6 (3840 x 3168)', 'x7 (4480 x 3696)', 'x8 (5120 x 4224)'],
      'x1 (640 x 528)'),
    sel('dolphin_widescreen_hack', 'Widescreen Hack 16:9', 'video',
      ['disabled', 'enabled'],
      'disabled'),
    ...
    sel('dolphin_antialiasing', 'Anti-Aliasing', 'video',
      ...
      'None'),
    sel('dolphin_force_texture_filtering', 'Forçar Filtro de Textura', 'video',
      ['disabled', 'enabled'],
      'disabled'),
    sel('dolphin_efb_copy_method', 'Cópia EFB', 'performance',
      ['EFB to Texture', 'EFB to RAM (HLE)', 'EFB to RAM (LLE)'],
      'EFB to Texture'),
    sel('dolphin_efb_skip_access', 'Skip EFB Access (CPU)', 'performance',
      ['disabled', 'enabled'],
      'disabled'),
    ...
    sel('dolphin_cpu_core', 'Core CPU', 'performance',
      ...
      'JIT64'),
    sel('dolphin_renderer', 'Renderer (Vulkan/OGL)', 'video',
      ['Hardware', 'Software', 'Null'],
      'Hardware'),
    sel('dolphin_audio_mixer_rate', 'Sample Rate Áudio', 'audio',
      ['32000 Hz (original)', '48000 Hz', '44100 Hz'],
      '32000 Hz (original)'),
    sel('dolphin_audio_stretch', 'Audio Stretch (anti-glitch)', 'audio',
      ['disabled', 'enabled'],
      'enabled'),
    ...
    sel('dolphin_pal60', 'PAL 60Hz', 'sistema',
      ['enabled', 'disabled'],
      'enabled'),
    sel('dolphin_language', 'Idioma Console', 'sistema',
      ['English', 'Portuguese', 'Spanish', 'French', 'German', 'Italian', 'Japanese'],
      'Portuguese'),
    sel('dolphin_wiimote_continuous_scanning', 'Wiimote Continuous Scan', 'input',
      ['disabled', 'enabled'],
      'disabled'),
  ],
  ...
  gba: [
    ...
    sel('mgba_interframe_blending', 'Inter-frame Blending', 'video',
      ...
      'OFF'),
    // v0.9.1: default era 'Remove Known' mas isso fazia GB acelerar em alguns jogos.
    // Default do mGBA upstream e 'Detect and Remove' (mais conservador, menos acel).
    // Se user quiser perf maxima, troca pra 'Remove Known' manualmente.
    sel('mgba_idle_optimization', 'Otimização Idle (CPU skip)', 'performance',
      ['Detect and Remove', 'Remove Known', 'Don\'t Remove'],
      'Detect and Remove'),
    ...
    sel('mgba_audio_low_pass_filter', 'Filtro Audio Low-Pass', 'audio',
      ['off', 'on'],
      'off'),
    sel('mgba_force_gbp', 'Forçar Game Boy Player', 'sistema',
      ['OFF', 'ON'],
      'OFF'),
    sel('mgba_sgb_borders', 'Bordas SGB (Super GameBoy)', 'video',
      ['ON', 'OFF'],
      'ON'),
  ],
...
export function getFrontendConfig(systemId) {
  ...
  return {
    ...
    ffSpeed:          parseFloat(values.ludex_fast_forward_speed) || 2,
